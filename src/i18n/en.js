...
module.exports = {
  ...
  'add.badSet': '`--set` needs key=value form: {tok}',
  'add.selectTitle': 'Choose {name}:',
  'add.missingField': 'Missing field "{name}" in non-interactive mode — pass `--set {name}=…`.',
  'add.missingSelect': 'Missing choice "{name}" in non-interactive mode — pass `--set {name}=…` (options: {valid}).',
  'add.invalidOption': '"{name}" has no option "{value}" (options: {valid}).',
  'add.unresolvedVar': 'Template env key "{key}" references undefined variable ${{var}}.',
  'add.listChoices': 'choices: {names}',
  ...
  'template.showInputs': 'Asked when creating:',
  ...
  'template.urlBadVar': 'URL "{url}" uses an unsupported variable (only "version" is allowed): {reason}',
