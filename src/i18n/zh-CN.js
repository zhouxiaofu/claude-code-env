...
module.exports = {
  ...
  'add.badSet': '`--set` 需要 key=value 形式：{tok}',
  'add.selectTitle': '选择 {name}：',
  'add.missingField': '非交互模式缺少字段 "{name}"，请用 `--set {name}=…` 提供。',
  'add.missingSelect': '非交互模式缺少选择 "{name}"，请用 `--set {name}=…` 提供（可选：{valid}）。',
  'add.invalidOption': '"{name}" 没有选项 "{value}"（可选：{valid}）。',
  'add.unresolvedVar': '模板 env 键 "{key}" 引用了未定义变量 ${{var}}。',
  'add.listChoices': '可选：{names}',
  ...
  'template.urlBadVar': 'URL "{url}" 含不支持的变量（仅支持 version）：{reason}',
