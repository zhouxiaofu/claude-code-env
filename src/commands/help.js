...
function helpEn() {
  return [
    ...
    `  -- <claude args...>     Pass everything after -- to claude (merged with config defaults)`,
    `  -o, --only              Drop config default args; use only what follows (with --, or none)`,
    `  -c, --continue          Continue the most recent conversation`,
    `  -r, --resume [id]       Resume a conversation (bare = interactive picker)`,
    ...
    `  add [tpl] [name]        Create an env from a template (interactive); --from <path|url> for an alt source`,
    `  remove, rm [name]       Remove an env (asks to confirm; -y to skip; no name = picker)`,
    `  template, tpl [...]     Manage templates: ls | show <n> | docs <n> | refresh | url | offline`,
    ...
    `  pick [-o/-c/-r/-n/-m/-- ...]  Interactively pick an env, then launch claude`,
    ...
    `  cce -e kimi -- --permission-mode bypassPermissions       ${pc.dim('# merge extra claude args')}`,
    `  cce -e kimi -o -- --resume XYZ                           ${pc.dim('# only these args (drop config defaults)')}`,
    `  cce -e kimi -c                                           ${pc.dim('# continue the last conversation')}`,
    `  cce -e kimi -n data                                      ${pc.dim('# name this session "data"')}`,
...
function helpZh() {
  return [
    ...
    `  -- <claude 参数...>     -- 之后的参数原样透传给 claude（与配置默认合并）`,
    `  -o, --only              丢弃配置默认参数，只用后面给的（配合 --，或不带）`,
    `  -c, --continue          继续最近一次对话`,
    `  -r, --resume [id]       恢复某次对话（裸写 = 交互选择）`,
    `  -n, --name <名称>       给本次会话设一个显示名`,
    ...
    pc.bold('子命令'),
    `  list, ls                列出所有 env`,
    `  add [模板] [名称]       从模板创建 env（交互式）；--from <路径|URL> 指定其它来源`,
    `  remove, rm [名称]       删除一个 env（会二次确认；-y 跳过；不带名称则弹出选择菜单）`,
    `  template, tpl [...]     管理模板：ls | show <名> | docs <名> | refresh | url | offline`,
    ...
    `  pick [-o/-c/-r/-n/-m/-- ...]  交互式选择 env，然后启动 claude`,
    ...
    `  cce -e kimi -- --permission-mode bypassPermissions       ${pc.dim('# 合并额外 claude 参数')}`,
    `  cce -e kimi -o -- --resume XYZ                           ${pc.dim('# 只用这些参数（丢弃配置默认）')}`,
    `  cce -e kimi -c                                           ${pc.dim('# 继续上次对话')}`,
    `  cce -e kimi -n data                                      ${pc.dim('# 把本次会话命名为 data')}`,
