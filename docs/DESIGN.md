...
> **更新（2026-07，已取代下文的 `-a`/`-A` 模型）：** `-a`/`-A` 已移除，改为正交模型 ——
> `--` 之后的参数透传给 claude（合并配置默认），`-o` / `--only` 丢弃配置默认，
> `-c`/`-r`/`-n` 是 claude `--continue`/`--resume`/`--name` 的一等化直达 flag。
> 敲旧的 `-a`/`-A` 会硬报错并打印新命令的改写。设计理由：`--` 是通用约定、避免引号二次切分；
> 把「传什么参数」（`--`）与「要不要丢默认」（`-o`）拆成两个正交开关，比 `-a`/`-A` 的大小写对仗更自解释。
> 详见 [usage.md → Claude 参数管理](usage.md#claude-参数管理)。下文保留原始设计作为历史记录。
