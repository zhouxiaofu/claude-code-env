...
根级 `env` 是所有配置共享的环境变量基底。具体 env 中的同名键会覆盖它；`null` 会在合并后转成空字符串，与直接填 `""` 一样表示显式清空。它是否覆盖 `settings.json` 中的同名值取决于 `settingsMode`：`override` / `cce` 下 CCE 优先，`claude` 下 `settings.json` 优先。最终结果仍只对本次启动的 `claude` 子进程生效。
